...
This document describes the simulations that extend the simple scenarios where the analytic or simple numerical solutions to determine how to allocate vaccination resources (i.e., pre-emptive, reactive vaccination, or the combination of boths), which were explored in the analytic_study.qmd document. 
...
compute_total_cost <- function(cells_dt,
    ...
    dt[, pre_red_used :=
        fifelse(
            pre_vax == TRUE,
            fifelse(
                is.na(get(pre_frac_red_col)), pre_red_default,
    ...
    dt[
        reactive_vax == TRUE & !is.na(get(react_frac_red_col)),
...
test_cost_vs_alpha_mc <- function(regions,
                                  ...
                                  seed_rng = 1) {
    stopifnot(
        is.list(regions), !is.null(regions$cells),
...
ggplot(df_sum, aes(x = alpha, y = total_cost_mean)) +
    ...
    geom_point(size = 2) +
    geom_errorbar(aes(
        ymin = total_cost_p2.5,
        ymax = total_cost_p97.5
    ), width = 0.02) +
    labs(
        x = expression(alpha ~ "(fraction allocated to pre-emptive)"),
        y = "Total cost",
