...
Severe outbreaks ($R = 6$) decrease the per-population expected cost as the vaccination capacity increases, to the point where all subpopulations are vaccinated ($f = 1$) for pre-emptive strategy. 

 favor pre-emptive vaccination unless the
...
*   **Scarce capacity ($f \le r/\nu$):** In this regime, the vaccine supply limits both strategies. The decision simplifies to comparing the expected cases prevented per dose: $\nu p$ for pre-emptive (discounted by risk) versus $r$ for reactive (targeted). Costs ($R$) cancel out because every available dose is used in either strategy.
*   **Abundant capacity ($f > r/\nu$):** Here, doses are not the limiting factor for the reactive strategy (which saturates at cost proportional to $p$). The decision is driven by minimizing the high cost of infections ($R$). As $R$ increases, the penalty for the "leaky" protection of reactive vaccination (which allows $1-r$ cases) outweighs the upfront cost of pre-emptive mass vaccination.
...
$$
C_{\mathrm{pre}}^{(n)} = \alpha f\, [C_V + p(1-\nu) C_I],
\qquad
...
-   outbreak with probability $p$: $C_V + (1-r)C_I$
-   no outbreak with probability $1-p$: $0$
...
Conditioning on an outbreak in the non-pre-emptive group: 


- with probability $q(\alpha)$: pay $C_V + (1-r)C_I$, 
