...
To achieve a target vaccine coverage proportion $C$ over a campaign duration $D$, the required daily rate $\rho$ (applied to the remaining eligible population) is determined by:
$$
\rho = -\frac{\ln(1 - C)}{D}
$$
...
ve_direct_val <- 0.7
ive_val <- 0.3 # Assumed indirect effectiveness for this example

# Overall effectiveness
cases_ove <- sapply(coverage_levels, function(cov) {
    calculate_cases_overall_effectiveness(
        cases_baseline = baseline_cases,
        coverage = cov,
        ve_direct = ve_direct_val,
        ve_indirect = ive_val
    )$cases_post
})

results_ove <- data.frame(
    coverage = coverage_levels,
    cases = cases_ove,
    dve = ve_direct_val,
    ive = ive_val,
    ove = 1 - cases_ove / baseline_cases
)

knitr::kable(results_ove, digits = 3, caption = "OVE calculation example")
```
