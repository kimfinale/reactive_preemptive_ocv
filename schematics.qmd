...
d_tradeoff <- bind_rows(
    tibble(week = -15:-1, `No vacc` = round(rnorm(15, 0, 3))),
    d
)
d_tradeoff$`No vacc` <- pmax(0, d_tradeoff$`No vacc`)

max_cases <- max(d_tradeoff$`No vacc`)
arrow_y_1 <- max_cases * 1.6
arrow_y_2 <- max_cases * 1.8
...
plt_tradeoff <- ggplot(d_tradeoff, aes(x = week, y = `No vacc`)) +
    ...
    annotate("text",
        ...
        label = "impact of vaccination decreases", size = annot_text_size * 1.2, color = "firebrick"
    ) +

    # Leftward arrow: Certainty decreases
    geom_segment(
        aes(x = -12, xend = 20, y = arrow_y_2, yend = arrow_y_2),
        arrow = arrow(length = unit(0.2, "cm"), type = "open"),
    ...
    annotate("text",
        ...
        size = annot_text_size * 1.2, color = "steelblue"
    ) +
    # --- Foreshadowing Intervention Windows ---
    # Pre-emptive window
    geom_segment(aes(x = -5, xend = -5, y = 0, yend = arrow_y_1 - 10), linetype = "dotted", color = "darkred", linewidth = 1) +
    annotate("text", x = -5.5, y = arrow_y_1 - 30, label = "pre-emptive\nwindow", hjust = 1, size = annot_text_size, color = "darkred") +
    # Reactive window
    geom_segment(aes(x = 5, xend = 5, y = 0, yend = arrow_y_1 - 10), linetype = "dotted", color = "steelblue", linewidth = 1) +
    annotate("text",
        x = 5.5, y = arrow_y_1 - 30, label = "reactive\nwindow",
        hjust = 0, size = annot_text_size, color = "steelblue"
    ) +
    labs(x = xlabel, y = ylabel) +
    scale_y_continuous(limits = c(0, arrow_y_2 + text_v_shift + 20)) +
...
plt_react <- ggplot(dlong, aes(week, value, linetype = name, shape = name, group = name)) +
    ...
    #     x = week_or - 0.5, y = 38,
    #     label = "outbreak\nresponse triggered",
    #     hjust = 0.5, size = annot_text_size, lineheight = line_height
    # ) +
...
p_area_pre <- plot_districts("Pre-emptive", pre_vax_ids, "firebrick") +
    # Legend: Outbreaks
    annotate("text", x = 0.8, y = annot_y, label = "*", size = 16, vjust = 0.8) +
    annotate("text", x = 0.95, y = annot_y, label = "outbreaks", hjust = 0, size = annot_text_size) +
    # Legend: Targeted
    annotate("tile", x = 2.2, y = annot_y, fill = alpha("firebrick", 0.4), height = 0.3, width = 0.3) +
    annotate("text", x = 2.45, y = annot_y, label = "pre-emptive\nvaccination", hjust = 0, size = annot_text_size, lineheight = 0.9, vjust = 0.5) +
    coord_fixed(ylim = c(0.6, 3.6), clip = "off")
# Reactive: Target Outbreaks only + Legend Item
p_area_react <- plot_districts("Reactive", outbreak_ids, "steelblue") +
    # Legend: Targeted
    annotate("tile", x = 1.0, y = annot_y, fill = alpha("steelblue", 0.4), height = 0.3, width = 0.3) +
    annotate("text", x = 1.25, y = annot_y, label = "reactive\nvaccination", hjust = 0, size = annot_text_size, lineheight = 0.9) +
    coord_fixed(ylim = c(0.6, 3.6), clip = "off")
...
plt_cost_single <- ggplot(df_cost_sloped, aes(x = Probability, y = Cost, color = Strategy)) +
    ...
    annotate("text",
        x = 0.1, y = 65, label = "pre-emptive",
        color = "firebrick", hjust = 0, size = annot_text_size
    ) +
    annotate("text",
        x = 0.74, y = 100, label = "reactive",
        color = "steelblue", hjust = 0.5, size = annot_text_size
    ) +
    theme_classic(base_size = theme_base_size) +
